        ...
        public async Task ReturnedRowCountsAsync()
        {
            ...
            await RunAsyncDynamicWriterVariants(
                ...
                async (config, getWriter, getStr) =>
                {
                    ...
                    await using (var csv = config.CreateAsyncWriter(writer))
                    {
                        var enumerable = new TestAsyncEnumerable<dynamic>(Enumerable.Range(0, 10).Select(x => MakeDynamicRow($"Item1,Item2\r\n{x + "-" + x},{x * 2}")), false);

                        var n = await csv.WriteAllAsync(enumerable);
