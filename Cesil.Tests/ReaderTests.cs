...
#pragma warning disable IDE1006
    ...
    {
        private sealed class _InstanceProviderResetNullabilityMismatch
        {

        }

        [Fact]
        public void InstanceProviderResetNullabilityMismatch()
        {
#nullable enable
            var ip =
                InstanceProvider.ForDelegate(
                    (in ReadContext ctx, out _InstanceProviderResetNullabilityMismatch? res) =>
                    {
                        res = null;
                        return true;
                    }
            ...
            var r = 
                Reset.ForDelegate(
                    (_InstanceProviderResetNullabilityMismatch row, in ReadContext ctx) =>
                    {
...
#nullable disable
            var s =
                Setter.ForDelegate(
                    (int val, in ReadContext ctx) => { }
                );

            var tb =
                ManualTypeDescriber.CreateBuilder()
                .WithInstanceProvider(ip)
                .WithExplicitSetter(typeof(_InstanceProviderResetNullabilityMismatch).GetTypeInfo(), "foo", s, Parser.GetDefault(typeof(int).GetTypeInfo()), MemberRequired.No, r)
                .ToManualTypeDescriber();

            var opts = Options.CreateBuilder(Options.Default).WithTypeDescriber(tb).ToOptions();

            var exc = Assert.Throws<InvalidOperationException>(() => Configuration.For<_InstanceProviderResetNullabilityMismatch>(opts));
            Assert.StartsWith("InstanceProvider ", exc.Message);
            Assert.Contains("to create Cesil.Tests.ReaderTests+_InstanceProviderResetNullabilityMismatch (AllowNull) may provide a null row, which Reset ", exc.Message);
            Assert.EndsWith(" will not accept.", exc.Message);
        }

        private sealed class _InstanceProviderSetterNullabilityMismatch
        {

        }

        [Fact]
        public void InstanceProviderSetterNullabilityMismatch()
        {
#nullable enable
            var ip =
                InstanceProvider.ForDelegate(
                    (in ReadContext ctx, out _InstanceProviderSetterNullabilityMismatch? res) =>
                    {
                        res = null;
                        return true;
                    }
            ...
            var s =
                Setter.ForDelegate(
                    (_InstanceProviderSetterNullabilityMismatch row, int val, in ReadContext ctx) => { }
                );
#nullable disable
            ...
            var tb =
                ManualTypeDescriber.CreateBuilder()
                .WithInstanceProvider(ip)
                .WithExplicitSetter(typeof(_InstanceProviderSetterNullabilityMismatch).GetTypeInfo(), "foo", s)
                .ToManualTypeDescriber();

            var opts = Options.CreateBuilder(Options.Default).WithTypeDescriber(tb).ToOptions();

            var exc = Assert.Throws<InvalidOperationException>(() => Configuration.For<_InstanceProviderSetterNullabilityMismatch>(opts));
            Assert.StartsWith("InstanceProvider ", exc.Message);
            Assert.Contains("to create Cesil.Tests.ReaderTests+_InstanceProviderSetterNullabilityMismatch (AllowNull) may provide a null row, which Setter ", exc.Message);
            Assert.EndsWith(" will not accept.", exc.Message);
        }

        [Fact]
        public void SetterParserNullabilityMismatch()
        {
#nullable enable
            var p =
                Parser.ForDelegate(
                    (ReadOnlySpan<char> r, in ReadContext ctx, out object? res) =>
                    {
                        res = null;
                        return true;
                    }
            ...
            var s =
                Setter.ForDelegate(
                    (int row, object val, in ReadContext ctx) => { }
                );
#nullable disable

            var ip = 
                InstanceProvider.ForDelegate(
                    (in ReadContext ctx, out int res) => { res = 0; return true; }
                );

            var exc = Assert.Throws<InvalidOperationException>(() => DeserializableMember.Create(typeof(int).GetTypeInfo(), "foo", s, p, MemberRequired.No, null));
            Assert.Equal($"Provided {nameof(Parser)} may return null values, which provided {nameof(Setter)} does not accept.", exc.Message);
        }
